...
def write_dss(dst: PathLike, df: pd.DataFrame):
    ...
    if missing:
        raise ValueError(
            f"Missing the following columns:\n{missing}\ngiven: {df.columns}"
        )
    # Validate E parts
    if "E" in df.columns:
    ...
    with pyhecdss.DSSFile(str(dst), create_new=True) as DST:
        for path, ts in df.groupby("PATH"):
            ...
            if len(period_type) != 1:
                raise ValueError(
                    f"Path {path} contains non-unique period-type: {period_type}"
                )
            data = ts["VALUE"]
            ...
            if data.index.freq is None:
                ...
                if freq is None:
                    raise ValueError(
                        f"Cannot determine frequency from index for {path=}"
                    )
                data.index.freq = pd.tseries.frequencies.to_offset(freq)
            DST.write_rts(pathname=path, df=data, cunits=units[0], ctype=period_type[0])
